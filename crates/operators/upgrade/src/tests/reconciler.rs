...
use kernel_controller::{
    Backoff, ControllerRuntime, FencedStore, LeaderIdentity, LeadershipToken, RuntimeConfig,
    TimestampClock,
};
use kernel_store::{
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(world.backend.requests().len(), 2);
    assert_eq!(
        world.backend.requests().first(),
        world.backend.requests().get(1)
    );
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(world.backend.requests().is_empty());
    Ok(())
}
...
impl World {
    async fn new(mutate_on_first_dispatch: bool) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        put_resource(&store, &keys, "UpgradeRun", &run()?).await?;
        let backend = Arc::new(RecordingBackend {
            store: store.clone(),
            keys: keys.clone(),
            mutate_on_first_dispatch,
            requests: Mutex::new(Vec::new()),
        });
        let reconciler = Arc::new(UpgradeReconciler::new(
            cluster_id,
            clock,
    ...
    async fn pass(&self) -> Result<(), Box<dyn std::error::Error>> {
        self.runtime.reconcile_snapshot().await?;
        Ok(())
    }

    async fn run(&self) -> Result<UpgradeRun, Box<dyn std::error::Error>> {
        self.run_optional()
            .await?
            .ok_or_else(|| "run missing".into())
    }

    async fn run_optional(&self) -> Result<Option<UpgradeRun>, Box<dyn std::error::Error>> {
    ...
    async fn maintained_nodes(&self) -> Result<BTreeSet<NodeId>, Box<dyn std::error::Error>> {
    ...
    async fn mark_run_deleting(&self) -> Result<(), Box<dyn std::error::Error>> {
        let key = self.keys.resource(
            &ResourceKind::new("UpgradeRun")?,
            &ResourceName::new("upgrade-1")?,
        );
...
impl NodeUpgradeBackend for RecordingBackend {
    async fn apply(&self, request: &NodeUpgradeRequest) -> Result<(), NodeUpgradeBackendError> {
        ...
        if first && self.mutate_on_first_dispatch {
