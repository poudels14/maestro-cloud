...
use crate::dns_resources::{
    DNS_RESOLVER_SERVICE_ID, DnsResolverResourceError, DnsResolverSystemResources, is_managed,
};

const MAXIMUM_CONFLICT_RETRIES: usize = 8;

pub(crate) struct DnsResolverResourceReconciler {
    service_id: ServiceId,
    service_key: StoreKey,
    desired: Option<DnsResolverSystemResources>,
}

impl DnsResolverResourceReconciler {
    pub(crate) fn new(
        cluster_id: &kernel_api::ClusterId,
        desired: Option<DnsResolverSystemResources>,
    ) -> Result<Self, DnsResolverReconcileError> {
    ...
    pub(crate) async fn reconcile(
        ...
        for _ in 0..MAXIMUM_CONFLICT_RETRIES {
            ...
            let value = serde_json::to_vec(&replacement).map_err(|error| {
                DnsResolverReconcileError::Serialize {
        ...
        Err(DnsResolverReconcileError::ConflictExhausted)
    }

    async fn read(
        &self,
        store: &FencedStore,
    ) -> Result<Option<StoredService>, DnsResolverReconcileError> {
        let Some(stored) = store.get(&self.service_key).await? else {
            return Ok(None);
        };
        let resource = serde_json::from_slice(&stored.value).map_err(|error| {
            DnsResolverReconcileError::Malformed {
...
fn converge(
    ...
    now: Timestamp,
) -> Result<Option<Service>, DnsResolverReconcileError> {
    match (current, desired) {
        ...
        (Some(current), Some(desired)) => {
            if !is_managed(&current.resource.meta.annotations) {
                return Err(DnsResolverReconcileError::ResourceCollision);
            }
            if current.resource.meta.deletion_timestamp.is_some() {
                return Err(DnsResolverReconcileError::ResourceTerminating);
            }
            ...
            if replacement.spec != desired.spec {
                replacement.meta.generation = Generation(
                    replacement
                        ...
                        .0
                        .checked_add(1)
                        .ok_or(DnsResolverReconcileError::GenerationExhausted)?,
                );
...
pub(crate) enum DnsResolverReconcileError {
    #[error(transparent)]
    Controller(#[from] ControllerError),
    #[error(transparent)]
    Store(#[from] kernel_store::StoreError),
    #[error(transparent)]
    Resource(#[from] DnsResolverResourceError),
    #[error(transparent)]
    Identifier(#[from] kernel_api::InvalidIdentifier),
    #[error("stored DNS resolver Service `{key}` is malformed: {message}")]
    Malformed { key: String, message: String },
    #[error("stored DNS resolver Service identity is `{actual}`, expected `{expected}`")]
    IdentityMismatch { expected: String, actual: String },
    #[error("built-in DNS resolver Service is owned by another writer")]
    ResourceCollision,
    #[error("built-in DNS resolver Service is still terminating")]
    ResourceTerminating,
    #[error("built-in DNS resolver Service exhausted its generation")]
    GenerationExhausted,
    #[error("failed to serialize built-in DNS resolver Service: {message}")]
    Serialize { message: String },
    #[error("built-in DNS resolver Service changed during every reconciliation attempt")]
    ConflictExhausted,
}
