...
mod tests {
    ...
    #[test]
    fn explicit_and_legacy_start_forms_select_the_same_config()
    -> Result<(), Box<dyn std::error::Error>> {
        let explicit = Cli::try_parse_from(["daemon", "start", "/run/maestro.json"])?;
        let legacy = Cli::try_parse_from(normalize_arguments(
            ["daemon", "/run/maestro.json"]
                .into_iter()
                .map(OsString::from)
                .collect(),
        ))?;
        assert!(matches!(
            ...
            DaemonCommand::Start { config }
                if config.as_path() == std::path::Path::new("/run/maestro.json")
        ));
        assert!(matches!(
            legacy.command,
            DaemonCommand::Start { config }
    ...
    #[test]
    fn dead_letter_commands_require_explicit_purge_scope() -> Result<(), Box<dyn std::error::Error>>
    {
        assert!(
            Cli::try_parse_from(["daemon", "dead-letters", "/run/maestro.json", "purge"]).is_err()
        );
        for scope in [["--all", ""], ["--through-seq", "42"]] {
