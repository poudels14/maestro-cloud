...
pub(crate) struct CutoverBundleOptions {
    ...
    pub(crate) operator_secret_file: PathBuf,
...
pub(crate) async fn prepare_cutover_bundle(
    ...
    let operator_jwt_secret = read_secret(&options.operator_secret_file, "operator JWT secret")?;
...
fn validate_paths(options: &CutoverBundleOptions) -> Result<(), CliError> {
    for (description, path) in [
        ...
        (
            ...
            options.operator_secret_file.as_path(),
        ),
