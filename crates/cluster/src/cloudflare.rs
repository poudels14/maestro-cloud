...
/// Default number of highly available Cloudflare Tunnel connectors per workload node.
pub const DEFAULT_CLOUDFLARE_TUNNEL_REPLICAS: u32 = 2;
/// Maximum number of Cloudflare Tunnel connectors per workload node.
pub const MAX_CLOUDFLARE_TUNNEL_REPLICAS: u32 = 25;
const MAXIMUM_TUNNEL_TOKEN_BYTES: usize = 16 * 1_024;

/// Remotely managed Cloudflare Tunnel connector policy applied to every workload node.
...
impl CloudflareTunnelConfig {
    /// Rejects unusable secrets and unbounded per-node connector counts.
...
pub enum CloudflareTunnelConfigError {
    ...
    ZeroReplicas,
    /// Per-node connector counts remain within the reviewed operational bound.
    #[error("tunnel replicas per node {replicas} exceed the maximum {maximum}")]
