...
async fn append_and_read_preserves_attrs_and_tags() {
    ...
    let entries = store.read_after(0, 10).await.expect("read");
...
async fn spool_cleanup_is_bounded_by_the_slowest_registered_sink() {
    ...
    let remaining = store.read_after(0, 10).await.expect("remaining");
    assert_eq!(
    ...
    assert!(store.read_after(0, 10).await.expect("empty").is_empty());
...
async fn migration_renames_legacy_attributes_column_to_tags() {
    ...
    let entries = store.read_after(0, 10).await.expect("read after migration");
    assert_eq!(entries.len(), 1);
    ...
    let entries = store.read_after(0, 10).await.expect("read again");
...
#[tokio::test]
async fn open_drops_retired_telemetry_tables_without_touching_spool_state() {
    let path = temp_db_path("retired-telemetry");
    let store = LogStore::open(&path).expect("open store");
    store.append(&[sample_entry()]).await.expect("append log");
    store
        .register_sink("controller")
        .await
        .expect("register sink");
    store
        .set_sink_cursor("controller", 1)
        .await
        .expect("advance sink");
    store
        .record_sink_dead_letter("datadog", 1, 400, "bad", b"[]")
        .await
        .expect("record dead letter");
    drop(store);

    {
        let conn = rusqlite::Connection::open(&path).expect("seed retired tables");
        conn.execute_batch(
            "CREATE TABLE ingress_traffic (value INTEGER);
             CREATE TABLE metrics (value INTEGER);
             CREATE TABLE traffic_metrics (value INTEGER);
             CREATE TABLE stats_metrics (value INTEGER);",
        )
        .expect("create retired tables");
    }

    let store = LogStore::open(&path).expect("reopen store");
    let entries = store.read_after(0, 10).await.expect("read spool");
    assert_eq!(entries.len(), 1);
    assert_eq!(
        store
            .get_sink_cursor("controller")
            .await
            .expect("read cursor"),
        1
    );
    ...
    let conn = rusqlite::Connection::open(&path).expect("inspect schema");
    for table in [
        "ingress_traffic",
        "metrics",
        "traffic_metrics",
